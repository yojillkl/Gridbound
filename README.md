...
关卡已改为一张 32×32 的连通地图：两处渡口、遗迹阶梯及土墙登高捷径；取回晶核并返回营地即可通关，无需清空守卫。界面只有中文玩法介绍和血量，取消小地图和统计面板。地图总格数从 400 增至 1024。

编译和 8 项自动化测试已通过；本地打包状态见 [构建记录](Docs/BuildStatus-v0.3.0.md)。公开下载未在本次修改中更新，仍为 v0.2.0。[新关卡设计](Docs/LevelDesign.md)。

本地双击项目上级目录的 `启动最新版Gridbound.cmd`，或运行 `Releases/v0.3.0/Windows/Play-Gridbound.cmd`。

![新版中文玩法说明与生命值](Validation/UnifiedLevel/ChineseHUD.png)

新版玩法：从绿色营地出发，经草桥或远处砂石渡口到达遗迹，跳上阶梯或用土墙托举登高。靠近金色晶核按 F 拾取，然后选择返程路线，回营地按 F 撤离。死亡会掉落晶核，重生后可以回收。火焰封路、泥地减速和土墙隔敌也会改变自己的退路；绕岩石脱离视线，离开橙色预警格可躲避劈砍。
...
- **破坏与 AI**：火球拆掉单根土柱；战士会追击、绕墙，无法绕行时劈砍拆墙。玩家死亡后回营地重生，掉落的晶核可回收；无需清敌即可撤离。
