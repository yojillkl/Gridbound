...
{
    for(int X=0;X<GridRules::BoardSize;++X) for(int Y=0;Y<GridRules::BoardSize;++Y)
    {
    ...
    TestFalse(TEXT("Upper boundary outside board"),GridRules::Inside(FIntPoint(GridRules::BoardSize,19)));
    ...
    TestEqual(TEXT("All cells have a terrain type"),Grass+River+Gravel,GridRules::BoardSize*GridRules::BoardSize);
    TestEqual(TEXT("Two-cell-wide river with a two three-cell-wide crossings"),River,52);
