...
> 招聘官快速体验：下载 Release 中的 `Gridbound-Windows-v0.1.0.zip`，完整解压，双击 `Play-Gridbound.cmd`。无需安装 Unreal Editor。此仓库主页展示项目与代码，试玩版为 Windows 下载运行形式。

![降雨、泥地与低多边形场景](Validation/Rain/RainArt.png)

*技能展示场景：降雨、泥地与格子地形。普通开局为战士追击战斗。*
...
- **破坏与 AI**：火球拆掉单根土柱；战士会追击、绕墙，无法绕行时劈砍拆墙。玩家死亡后重生，敌人全灭后补充一名战士。
...
| 重生 | 玩家 100 HP，死亡 2 秒后重生；清场后补充 1 名战士；出生点被占用时寻找附近安全格 |
...
- 7 组 Unreal 自动化测试通过，覆盖技能、地形、移动、跳跃、战士和重生；测试源码位于 `Source/Gridbound/*Tests.cpp`。
- Windows 独立试玩包已通过 DX12 启动、地图与材质加载、截图及退出检查；详细验证记录见 [开发记录](Docs/DevelopmentNotes.md)。
- 当前为单人战斗原型：一个场地、三种玩家技能、一种敌人；尚无主菜单、存档、音效或联网功能。HUD 暂用英文，土墙预览仍使用调试绘制。
- 发布版为 Development 构建以保留放置预览；未宣称完成不同硬件上的兼容性和性能测试。
